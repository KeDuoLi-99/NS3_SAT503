...
#include "../model/geo-channel-constant.h"
...
LeoChannelHelper::SetConstellation (std::string constellation)
{
  ...
  else if (constellation == "GeoEasyGateway")
    {
      //geo简单信道模型
      std::cout << "GeoEasyGateWay" << std::endl;
      SetConstellationAttributes (GEO_EASY_GATEWAY_EIRP,
      GEO_EASY_GATEWAY_ELEVATION_ANGLE,
      GEO_EASY_GATEWAT_FSPL,
      GEO_EASY_GATEWAT_ATMOSPHERIC_LOSS,
      GEO_EASY_GATEWAT_LINK_MARGIN,
      GEO_EASY_GATEWAT_DATA_RATE,
      GEO_EASY_GATEWAT_RX_ANTENNA_GAIN,
      0.0
      );
    }
  else
